...
def test():
	...
	save=[float(res),str(uw),now.year, now.month, now.day, now.hour, now.minute, now.second]
...
def get_signin():
	...
	for i in range(len(tk)):
		if username == tk['tk'][i] and password == str(tk['pass'][i]):
			...
			check=0
	if check==1:
		return redirect('https://fb-rec.herokuapp.com/?fbclid=IwAR0Zl5LzX_GNnKDuUqJgtoCJHoLgnkGMcZCIpj-r91ue7abJuI-5DsxDcf8')
	else:
